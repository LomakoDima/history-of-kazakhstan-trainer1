...
export async function POST(request) {
  try {
    ...
    if (data.status === "incomplete") {
      ...
      return reply({ error: "OpenAI response was incomplete (" + reason + "). Please run the review again." }, 502);
    }
    ...
    try {
      ...
      return reply({ error: "OpenAI returned an incomplete JSON review. Please run the review again." }, 502);
    }
  } catch (error) {
